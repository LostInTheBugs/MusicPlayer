...
- **Now playing panel (podcasts)**: title, description and **offline transcription** (Plugins ▸ Whisper models… — download official whisper.cpp models directly or add a local one, pick the default; live **progress bar + %** while transcribing, scrollable transcript below). Cloud API transcription and translation plugins are in development on the `pre-release` branch.
...
`2026.08.101-c2-pre` — see [CHANGELOG.md](CHANGELOG.md)
